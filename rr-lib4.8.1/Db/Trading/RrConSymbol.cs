...
namespace rrlib481.Db.Trading
{
    ...
    /// </summary>
    public record struct RrMTConSymbolSession
    {
        /// <summary>
    ...
    public record RrConSymbol : BaseDbRecord
    {
        /// <summary>
        /// Update from MTConSymbol
        /// </summary>
        /// <param name="symbol">CIMTConSymbol</param>
        public void UpdateFrom(CIMTConSymbol symbol)
        {
            LastUpdated = DateTime.Now;
            //Symbol = symbol.Symbol(); this shouldn't be changed
            Path = symbol.Path();
            ISIN = symbol.ISIN();
            ...
            TickFlags = (EnTickFlags)symbol.TickFlags();
            TickBookDepth = symbol.TickBookDepth();
            ChartMode = (EnChartMode)symbol.ChartMode();
            ...
            TradeMode = (EnTradeMode)symbol.TradeMode();
            TradeFlags = (EnTradeFlags)symbol.TradeFlags();
            CalcMode = (EnCalcMode)symbol.CalcMode();
            ExecMode = (EnExecutionMode)symbol.ExecMode();
            GTCMode = (EnGTCMode)symbol.GTCMode();
            FillFlags = (EnFillingFlags)symbol.FillFlags();
            OrderFlags = (EnOrderFlags)symbol.OrderFlags();
            ExpirFlags = (EnExpirationFlags)symbol.ExpirFlags();
            ...
            AccruedInterest = symbol.AccruedInterest();
            Category = symbol.Category();
            CFI = symbol.CFI();
            ContractSize = symbol.ContractSize();
            Country = symbol.Country();
            Exchange = symbol.Exchange();
            FaceValue = symbol.FaceValue();
            FreezeLevel = symbol.FreezeLevel();
            IECheckMode = (EnInstantMode)symbol.IECheckMode();
            IEFlags = (EnTradeInstantFlags)symbol.IEFlags();
            IESlipLosing = symbol.IESlipLosing();
            IESlipProfit = symbol.IESlipProfit();
            IETimeout = symbol.IETimeout();
            IEVolumeMax = symbol.IEVolumeMax();
            IEVolumeMaxExt = symbol.IEVolumeMaxExt();
            Industry = (EnIndustries)symbol.Industry();
            MarginHedged = symbol.MarginHedged();
            MarginInitial = symbol.MarginInitial();
            MarginMaintenance = symbol.MarginMaintenance();
            MarginRateCurrency = symbol.MarginRateCurrency();
            MarginRateLiquidity = symbol.MarginRateLiquidity();
            MarginFlags = (EnMarginFlags)symbol.MarginFlags();
            OptionsMode = (EnOptionMode)symbol.OptionsMode();
            PriceLimitMax = symbol.PriceLimitMax();
            PriceLimitMin = symbol.PriceLimitMin();
            PriceSettle = symbol.PriceSettle();
            PriceStrike = symbol.PriceStrike();
            QuotesTimeout = symbol.QuotesTimeout();
            REFlags = (EnRequestFlags)symbol.REFlags();
            RETimeout = symbol.RETimeout();
            Sector = (EnSectors)symbol.Sector();
            SessionsQuotes = null; // need to convert
            SessionsTrades = null; // need to convert
            SpliceTimeDays = symbol.SpliceTimeDays();
            SpliceTimeType = (EnSpliceTimeType)symbol.SpliceTimeType();
            StopsLevel = symbol.StopsLevel();
            SpliceType = (EnSpliceType)symbol.SpliceType();
            SwapLong = symbol.SwapLong();
            SubscriptionsDelay = symbol.SubscriptionsDelay();
            SwapShort = symbol.SwapShort();
            Swap3Day = (int)symbol.Swap3Day();
            SwapMode = (EnSwapMode)symbol.SwapMode();
            TickSize = symbol.TickSize();
            TickValue = symbol.TickValue();
            TimeExpiration = symbol.TimeExpiration();
            TimeStart = symbol.TimeStart();
            ...
            double rate = 0;
            #region MarginRateInitial Udpate
            MarginRateInitial.Clear();
            // MARGIN_RATE_BUY
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY, rate);
            }
            // MARGIN_RATE_SELL
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL, rate);
            }
            // MARGIN_RATE_BUY_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate);
            }
            // MARGIN_RATE_BUY_STOP
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate);
            }
            // MARGIN_RATE_SELL_STOP
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate);
            }
            // MARGIN_RATE_BUY_STOP_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_STOP_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate);
            }
            #endregion
            #region MarginRateMaintenance Update
            MarginRateMaintenance.Clear();
            // MARGIN_RATE_BUY
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY, rate);
            }
            // MARGIN_RATE_SELL
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL, rate);
            }
            // MARGIN_RATE_BUY_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate);
            }
            // MARGIN_RATE_BUY_STOP
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate);
            }
            // MARGIN_RATE_SELL_STOP
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate);
            }
            // MARGIN_RATE_BUY_STOP_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_STOP_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate);
            }
            #endregion
        }
        public static RrConSymbol CreateFrom(MTConSymbol symbol, string sid)
        {
            RrConSymbol rrConSymbol = new()
            {
                IsActive = true,
                LastUpdated = DateTime.Now,
                ServerID = sid,
                ...
                MarginRateCurrency = symbol.MarginRateCurrency,
                MarginRateLiquidity = symbol.MarginRateLiquidity,
                MarginRateInitial = symbol.MarginRateInitial.ToDictionary(kvp => (EnMarginRateTypes)kvp.Key, kvp => kvp.Value),
                MarginRateMaintenance = symbol.MarginRateMaintenance.ToDictionary(kvp => (EnMarginRateTypes)kvp.Key, kvp => kvp.Value),
                Multiply = symbol.Multiply,
                OptionsMode = (EnOptionMode)symbol.OptionsMode,
                OrderFlags = (EnOrderFlags)symbol.OrderFlags,
                Page = symbol.Page,
                ...
                RETimeout = symbol.RETimeout,
                Sector = (EnSectors)symbol.Sector,
                SessionsQuotes = symbol.SessionsQuotes.Select(l => l.Select(s => new RrMTConSymbolSession
                {
                    Open = s.Open,
                ...
                SessionsTrades = symbol.SessionsTrades.Select(l => l.Select(s => new RrMTConSymbolSession
                {
                    Open = s.Open,
                    OpenHours = s.OpenHours,
                    Close = s.Close,
        ...
        public static RrConSymbol CreateFrom(CIMTConSymbol symbol, string sid)
        {
            RrConSymbol rrConSymbol = new()
            {
                IsActive = true,
            ...
            double rate = 0;
            #region MarginRateInitial Initialisation
            // MARGIN_RATE_BUY
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY, rate);
            }
            // MARGIN_RATE_SELL
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL, rate);
            }
            // MARGIN_RATE_BUY_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate);
            }
            // MARGIN_RATE_BUY_STOP
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate);
            }
            // MARGIN_RATE_SELL_STOP
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate);
            }
            // MARGIN_RATE_BUY_STOP_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_STOP_LIMIT
            if (symbol.MarginRateInitial((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateInitial.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate);
            }
            #endregion
            #region MarginRateMaintenance Initialisation
            // MARGIN_RATE_BUY
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY, rate);
            }
            // MARGIN_RATE_SELL
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL, rate);
            }
            // MARGIN_RATE_BUY_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL_LIMIT, rate);
            }
            // MARGIN_RATE_BUY_STOP
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP, rate);
            }
            // MARGIN_RATE_SELL_STOP
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP, rate);
            }
            // MARGIN_RATE_BUY_STOP_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_BUY_STOP_LIMIT, rate);
            }
            // MARGIN_RATE_SELL_STOP_LIMIT
            if (symbol.MarginRateMaintenance((uint)EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate) == MTRetCode.MT_RET_OK)
            {
                rrConSymbol.MarginRateMaintenance.Add(EnMarginRateTypes.MARGIN_RATE_SELL_STOP_LIMIT, rate);
            }
            #endregion
            return rrConSymbol;
        }
        ...
        public static string GenerateCbKey(string sid, string symbol)
        {
            return $"{MethodBase.GetCurrentMethod().DeclaringType.Name}-{sid}-{symbol}";
        }
        ...
        public override string ChildGenerateCbKey()
        {
            ...
            return GenerateCbKey(ServerID, Symbol);
        }
        #endregion
        /// <summary>
        /// Minimal volume
        /// </summary>
        private ulong m_VolumeMin { get; set; } = 0;
        /// <summary>
        /// Maximum volume
        /// </summary>
        private ulong m_VolumeMax { get; set; } = 0;
        /// <summary>
        /// Volume step
        /// </summary>
        private ulong m_VolumeStep { get; set; } = 0;
        /// <summary>
        /// Volume limit
        /// </summary>
        private ulong m_VolumeLimit { get; set; } = 0;
        /// <summary>
        /// Instant execution maximum volume
        /// </summary>
        private ulong m_IEVolumeMax { get; set; } = 0;
        /// <summary>
        ...
        /// </summary>
        public Dictionary<EnMarginRateTypes, double> MarginRateInitial { get; set; } = new();
        /// <summary>
        /// orders and positions margin rates
        /// </summary>
        public Dictionary<EnMarginRateTypes, double> MarginRateMaintenance { get; set; } = new();
        /// <summary>
        /// orders and positions margin rates
        /// </summary>
        ...
        /// quote sessions
        /// </summary>
        public List<List<RrMTConSymbolSession>> SessionsQuotes { get; set; }
        /// <summary>
        /// trade sessions
        /// </summary>
        public List<List<RrMTConSymbolSession>> SessionsTrades { get; set; }
        /// <summary>
        /// request execution flags
        /// </summary>
