...
{
    public record RrOrder : BaseDbRecord
    {
        /// <summary>
        /// 用CIMTOrder更新RrOrder
        /// </summary>
        /// <param name="order">CIMTOrder</param>
        public void UpdateFrom(CIMTOrder order)
        {
            LastUpdated = DateTime.Now;
            //Order = order.Order(); this can't be changed
            ...
            State = (EnOrderState)order.State();
            Reason = (EnOrderReason)order.Reason();
            TimeSetup = order.TimeSetup();
            TimeExpiration = order.TimeExpiration();
            TimeDone = order.TimeDone();
            TimeSetupMsc = order.TimeSetupMsc();
            TimeDoneMsc = order.TimeDoneMsc();
            ModifyFlags = (EnTradeModifyFlags)order.ModificationFlags();
            Type = (EnOrderType)order.Type();
            TypeFill = (EnOrderFilling)order.TypeFill();
            TypeTime = (EnOrderTime)order.TypeTime();
            ...
            ExpertPositionID = order.PositionID();
            PositionByID = order.PositionByID();
            Comment = order.Comment();
            ActivationMode = (EnOrderActivation)order.ActivationMode();
            ActivationTime = order.ActivationTime();
            ActivationPrice = order.ActivationPrice();
            ActivationFlags = (EnTradeActivationFlags)order.ActivationFlags();
            VolumeCurrentExt = order.VolumeCurrentExt();
            VolumeInitialExt = order.VolumeInitialExt();
        }
        /// <summary>
        /// Create RrMTOrder from CIMTOrder
        /// </summary>
        /// <param name="order">MT5 CIMTOrder object</param>
        /// <param name="sid">Server ID</param>
        /// <returns>RrMTOrder</returns>
        public static RrOrder CreateFrom(CIMTOrder order, string sid)
        {
            RrOrder rrOrder = new()
            {
                IsActive = true,
        ...
        /// <summary>
        /// Create RrMTOrder from MTOrder
        /// </summary>
        /// <param name="order">MT5 MTOrder object</param>
        /// <param name="sid">Server ID</param>
        /// <returns>RrMTOrder</returns>
        public static RrOrder CreateFrom(MTOrder order, string sid)
        {
            RrOrder rrOrder = new()
            {
                IsActive = true,
                ...
                TimeSetupMsc = order.TimeSetupMsc,
                TimeDoneMsc = order.TimeDoneMsc,
                ModifyFlags = (EnTradeModifyFlags)order.ModifyFlags,
                Type = (EnOrderType)order.Type,
                TypeFill = (EnOrderFilling)order.TypeFill,
                TypeTime = (EnOrderTime)order.TypeTime,
                PriceOrder = order.PriceOrder,
                ...
                VolumeCurrent = order.VolumeCurrent,
                ExpertID = order.ExpertID,
                ExpertPositionID = order.ExpertPositionID,
                PositionByID = order.PositionByID,
                Comment = order.Comment,
                ActivationMode = (EnOrderActivation)order.ActivationMode,
                ActivationTime = order.ActivationTime,
        ...
        /// </summary>
        /// <param name="sid">Server ID</param>
        /// <param name="order">MT5 order ticket</param>
        /// <returns>Generated Key for Couchbase</returns>
        public static string GenerateCbKey(string sid, ulong order)
        {
            return $"{MethodBase.GetCurrentMethod().DeclaringType.Name}-{sid}-{order}";
        }
        #region Pure Copy, 複製區塊
        public override string ChildGenerateCbKey()
        {
            if (ServerID is null || string.IsNullOrEmpty(ServerID))
            {
                throw new ArgumentNullException("ServerID is null or empty");
            }
            if (Order == 0)
            {
                throw new ArgumentNullException("Order is null or zero");
            }
            return GenerateCbKey(ServerID, Order);
        }
        #endregion
        /// <summary>
        /// Initial Volume
        /// </summary>
        private ulong m_VolumeInitial { get; set; } = 0;
        /// <summary>
        /// Current Volume
        /// </summary>
        private ulong m_VolumeCurrent { get; set; } = 0;
        ...
        /// this is the PositionID in CIMTOrder
        /// </summary>
