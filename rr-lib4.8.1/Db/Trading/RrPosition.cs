...
{
    public record RrPosition : BaseDbRecord
    {
        public void UpdateFrom(CIMTPosition position)
        {
            ...
            Action = (EnPositionAction)position.Action();
            Digits = position.Digits();
            DigitsCurrency = position.DigitsCurrency();
            Reason = (EnPositionReason)position.Reason();
            ContractSize = position.ContractSize();
            TimeCreate = position.TimeCreate();
            TimeUpdate = position.TimeUpdate();
            ModifyFlags = (EnTradeModifyFlags)position.ModificationFlags();
            ...
            //Commission = position.Commission(); // Not available in CIMTPosition
            ...
            ActivationMode = (EnActivation)position.ActivationMode();
            ActivationTime = position.ActivationTime();
            ActivationPrice = position.ActivationPrice();
            ActivationFlags = (EnTradeActivationFlags)position.ActivationFlags();
            VolumeExt = position.VolumeExt();
        }
        /// <summary>
        /// Create RrPosition from MTPosition
        /// </summary>
        /// <param name="position">MT5 MTPosition </param>
        /// <param name="sid">Server ID</param>
        /// <returns>RrPosition</returns>
        public static RrPosition CreateFrom(MTPosition position, string sid)
        {
            return new()
            {
                ServerID = sid,
                ...
                TimeCreate = position.TimeCreate,
                TimeUpdate = position.TimeUpdate,
                ModifyFlags = (EnTradeModifyFlags)position.ModifyFlags,
                PriceOpen = position.PriceOpen,
                PriceCurrent = position.PriceCurrent,
                PriceSL = position.PriceSL,
                PriceTP = position.PriceTP,
        ...
        /// <summary>
        /// Create RrPosition from CIMTPosition
        /// </summary>
        /// <param name="position">MT5 CIMTPosition object</param>
        /// <param name="sid">Server ID</param>
        /// <returns>RrPosition</returns>
        public static RrPosition CreateFrom(CIMTPosition position, string sid)
        {
            return new()
            {
                ServerID = sid,
        ...
        /// </summary>
        /// <param name="sid">Server ID</param>
        /// <param name="position">MT5 position ticket</param>
        /// <returns>Generated Key for Couchbase</returns>
        public static string GenerateCbKey(string sid, ulong position)
        {
            return $"{MethodBase.GetCurrentMethod().DeclaringType.Name}-{sid}-{position}";
        }
        ...
        public override string ChildGenerateCbKey()
        {
            ...
            if (Position == 0)
            {
                throw new ArgumentNullException("Position is null or zero");
            }
            return GenerateCbKey(ServerID, Position);
        }
        #endregion
        /// <summary>
        /// Volume
